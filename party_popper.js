...
$(document).ready(function(){
	for(var i = 0; i < 100; i++){
		confettiColors.push(new __fillColor(Math.floor(Math.random()*256), Math.floor(Math.random()*256), Math.floor(Math.random()*256), 0.8));
	}
...
function PartyPopper(x1, y1, angle1, ctx1, color1){
	...
	image.onload = function(){
		...
		confetti[i] = new __ConfettiCircle(x+offsetY*Math.sin(angle*PI/180), y+offsetY*Math.cos(-angle*PI/180), r, angle);
		}
	};
	image.src = imageSrc;
	...
	function drawPopper(){
		...
		if(!popped){
			...
			var pip = ctx.isPointInPath(xCoordClick, yCoordClick);
			if(pip){
				popPopper();
			}
		}
...
function __ConfettiCircle(x, y, r, angle){
